...
public:
  vel_struct();
  void setVel( int, int, int, int, int );
  int star;
  int port;
  int top;
  int servo;  
...
void vel_struct :: setVel( int currStar, int currPort, int currTop, int currServo, int currLed)	{
  star = currStar;
  port = currPort;
  top = currTop;
  servo = currServo;
...
void controlCallback(const std_msgs::Int8::ConstPtr& mode)
{
  ...
  switch(curr_mode)	{
    case 0:
      curr_vel.setVel( STAR_REF_VEL, PORT_REF_VEL, TOP_REF_VEL, SERVO_REF_VEL, LED_REF);
      break;
    case 1:
    ...
    case -4:
      ...
      break;      
    case 9:
      if (prev_mode == curr_mode && curr_vel.led < LED_MAX)	curr_vel.led++;
      break;
    case -9:
      if (prev_mode == curr_mode && curr_vel.led > LED_MIN)	curr_vel.led--;
      break;      
  }
  prev_mode = curr_mode;
