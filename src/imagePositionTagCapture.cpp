...
char rightImageName[] = "right_image_";
char leftImageName[] = "left_image_";
char imageFoldername[] = "/home/prasanna/ros_hydro_arv/src/openrov/image_position_tag_data";
static const char LEFT_WINDOW[] = "Left image window";
static const char RIGHT_WINDOW[] = "Right image window";
char *leftFilename = (char*) calloc(256, sizeof(char));
char *rightFilename = (char*) calloc(256, sizeof(char));
...
cv_bridge::CvImagePtr leftCamImage, rightCamImage;
...
void tagCallback(const ar_track_alvar::AlvarMarkers& msg) {
  ...
  if (timeDiff > pauseTime && !msg.markers.empty()){
	  ...
	  sprintf(leftFilename, "%s/%s%05d.png",imageFoldername, leftImageName, imageCounter);
	  imwrite( leftFilename, leftCamImage->image );

	  sprintf(rightFilename, "%s/%s%05d.png",imageFoldername, rightImageName, imageCounter);
	  imwrite( rightFilename, rightCamImage->image );
  ...
  //Check if time counter

}

void leftImageCallback(const sensor_msgs::Image::ConstPtr& camImageMsg)	{
  //cout<<"Left image received ..."<<endl;

  try  {
    leftCamImage = cv_bridge::toCvCopy(camImageMsg, enc::BGR8);
  }
...
void rightImageCallback(const sensor_msgs::Image::ConstPtr& camImageMsg)	{
	  //cout<<"Right image received ..."<<endl;

	  try  {
	    rightCamImage = cv_bridge::toCvCopy(camImageMsg, enc::BGR8);
	  }
	  catch (cv_bridge::Exception& e)  {
	    ROS_ERROR("cv_bridge exception: %s", e.what());
	    return;
	  ...
	  //namedWindow(RIGHT_WINDOW);
	  //imshow(RIGHT_WINDOW, rightCamImage->image);
	  //waitKey(3);

	  //if(rightCamImage)	rightCamImage->image.release();
}
...
int main(int argc, char **argv) {
	...
	ros::Subscriber leftImageSub = n.subscribe("minoru/left/image_raw", 10, leftImageCallback);
	ros::Subscriber rightImageSub = n.subscribe("minoru/right/image_raw", 10, rightImageCallback);
